...
indices2 = [i for i, s in enumerate(k) if 'my_box' in s]
...
for j in xrange(0,len(coordinates)): #ignoring first 2 coordinates as they are the origin and the jackal locations!
    if j in indices2:  
      xleft=float(coordinates[j][0])-squre_length/2   
      yleft=float(coordinates[j][1])-squre_length/2
      rectx = patches.Rectangle((xleft,yleft),squre_length,squre_length,linewidth=2, edgecolor='black',facecolor='none')
      ax.add_patch(rectx)
